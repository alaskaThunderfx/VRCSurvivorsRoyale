...
public class LilGhost : UdonSharpBehaviour
{
    [Header("Components")]
    public NavMeshAgent Agent;
    public VRCPlayerApi Owner;
    private VRCPlayerApi LocalPlayer;
    public VRCObjectPool LilGhostPool;

    public Cyan.PlayerObjectPool.CyanPlayerObjectAssigner PlayerObjectAssigner;
    public PlayerController PlayerController;

    public LilGhostSpawner LilGhostSpawner;
    ...
    private void OnEnable()
    {
        ...
        HealthBarCanvas = transform.GetChild(18);
        HealthBar = HealthBarCanvas.GetChild(0).GetComponent<HealthBar>();
        LilGhostPool = GameObject.Find("LilGhostSpawner").GetComponent<VRCObjectPool>();
        LilGhostSpawner = GameObject.Find("LilGhostSpawner").GetComponent<LilGhostSpawner>();
        ...
        SpawnCountdown = 1.1f;
        IsDying = false;
        transform.GetChild(19).GetComponent<Collider>().enabled = true;
    }
        ...
        if (IsDying)
        {
            ...
            if (DeathScene <= 0)
            {
                ...
                if (Networking.LocalPlayer == Owner)
                {
                    ...
                    LilGhostPool.Return(gameObject);
                    LilGhostSpawner.NumberOfGhosts -= 1;
                }
            }
        }
        ...
        if (Agent.enabled)
        {
            ...
            switch (CurrentState)
            {
                ...
                case 1:
                    // if (!Networking.IsOwner(gameObject))
                    //     break;
                    transform.GetChild(19).GetComponent<Collider>().enabled = false;
    ...
    public float Health
    {
        set
        {
            ...
            if (health <= 0)
            {
                ...
                Networking.SetOwner(Owner, LilGhostPool.gameObject);
                Agent.enabled = false;
